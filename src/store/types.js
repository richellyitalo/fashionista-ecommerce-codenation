// app
export const APP_SET_OVERLAY = '@app/SET_OVERLAY';
export const APP_CLEAR_OVERLAY = '@app/CLEAR_OVERLAY';

// produtos
export const PRODUTO_SET_PRODUTOS = '@produto/SET_PRODUTOS';
export const PRODUTO_GET_PRODUTO = '@produto/GET_PRODUTO';
export const PRODUTO_SET_DETAIL = '@produto/SET_DETAIL';

// cart
export const CART_ADD_PRODUTO = '@cart/ADD_PRODUTO';
export const CART_CLEAR = '@cart/CLEAR';
export const CART_DECREASE_PRODUTO = '@cart/DECREASE_PRODUTO';
export const CART_REMOVE_ITEM = '@cart/REMOVE_ITEM';
